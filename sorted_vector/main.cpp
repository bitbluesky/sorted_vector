...
{
    typename codeproject::sorted_vector<int,true> svec(v0.begin(),v0.end());
...
{
    typename codeproject::sorted_vector<int,true> svec(v0.begin(),v0.end());
    typename codeproject::sorted_vector<int,true>::Cont& vInterSect
...
{   
    typename codeproject::sorted_vector<T,true> svec;
    ...
    typename codeproject::sorted_vector<T,true> svec1;
...
{
    typename codeproject::sorted_vector<T> svec;
    {SimpTimer t("build sortedvec_multiset (naiv 每次插入都是sorted vector有序插入)");
        for(unsigned i=0;i<v.size();i++){
            svec.insert(v[i]);
        }
    }
    codeproject::sorted_vector<T> svec1;
    {SimpTimer t("build sortedvec_multiset (opt. push_back全部插入后，一次排序)");
